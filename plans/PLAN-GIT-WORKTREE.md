...
- код и планы (`plans/`) версионируются по веткам, как сейчас;
...
`plans/`, `src/`, `public/`, конфиги (`vite.config.ts`, `vitest.config.ts`, `tsconfig*.json`,
...
- В §3 «Where things are» добавить блок раскладки контейнера и пометку, что пути
  `src/…`, `plans/…` относятся к текущему worktree.
- Новый подраздел «Worktrees» с командами создания/удаления и правилами:
  - `plans/` и код версионируются по веткам;
