...
export default function TargetCanvas({
  ...
  return (
    <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 8, touchAction: 'none' }}>
      <svg
        ref={svgRef}
        viewBox={`0 0 ${VIEW} ${VIEW}`}
        style={{ width: '100%', maxWidth: '100%', aspectRatio: '1', touchAction: 'none' }}
      ...
      >
        ...
        {shots.map(shot => {
          ...
          if (isDragging) {
            // Dragging shot: medium, highlighted
            return <circle key={shot.id} cx={sp.px} cy={sp.py} r={1.5} fill="red" stroke="white" strokeWidth={0.3} />;
          }
          if (isLast) {
            // Last shot: larger, different color
            return <circle key={shot.id} cx={sp.px} cy={sp.py} r={1.2} fill="#e74c3c" stroke="white" strokeWidth={0.2} />;
          }
          // Other shots: small, semi-transparent
          return <circle key={shot.id} cx={sp.px} cy={sp.py} r={0.8} fill="rgba(30,30,30,0.5)" />;
        })}
      </svg>
    </div>
  );
