...
export default function TrainingScreen({ athlete, training, epoch, onBack }: Props) {
  ...
  const toggleZoom = async () => {
    const next = zoomMode === 'full' ? 'zoom7' : 'full';
    setZoomMode(next);
    const db = await openDB();
...
const s: Record<string, React.CSSProperties> = {
  ...
  zoomToggle:    { background: 'none', border: '1px solid #ccc', borderRadius: 6, fontSize: 16, padding: '4px 8px', cursor: 'pointer', lineHeight: 1, minWidth: 32, textAlign: 'center' as const },
